...
(function( $ ) {
    ...
    redux.field_objects.link_color.init = function( selector ) {
        ...
        if ( !selector ) {
            selector = $( document ).find( '.redux-container-link_color:visible, .redux-container-nav_color:visible' );
        }
            ...
            function() {
                ...
                $control.each(function() {
                	...
                	$this.wpColorPicker({
		                ...
		                $('<div class="vc_alpha-container">' + '<label>Alpha: <output class="rangevalue">' + alpha_val + '%</output></label>' + '<input type="range" min="1" max="100" value="' + alpha_val + '" name="alpha" class="vc_alpha-field">' + '</div>').appendTo($this.parents('.wp-picker-container:first').addClass('vc_color-picker').find('.iris-picker'));
		                $alpha = $this.parents('.wp-picker-container:first').find('.vc_alpha-field');
		                $alpha_output = $this.parents('.wp-picker-container:first').find('.vc_alpha-container output')
		                $alpha.bind('change keyup', function() {
		                    var alpha_val = parseFloat($alpha.val()),
		                        iris = $this.data('a8cIris'),
		                        color_picker = $this.data('wpWpColorPicker');
		                    $alpha_output.val($alpha.val() + '%');
		                    iris._color._alpha = alpha_val / 100.0;
		                    $this.val(iris._color.toString());
