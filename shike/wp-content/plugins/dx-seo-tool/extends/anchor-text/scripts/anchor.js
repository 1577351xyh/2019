jQuery(document).ready(function($){
    ...
    /* 生成专题页 */
    $('.generate_topic_pages').live('click',function(){

        if( confirm( '确认要生成专题页吗?操作不可逆，请谨慎操作。' ) ) {

            $.post(
                ajaxurl,
                {
                    _nonce: anchorNonce,
                    action: dxseoAnchor.pre + '_anchor_generate',
                    keyword: $(this).parents( 'tr.item' ).find( '.keyword' ).text()
                },
                function(res){
    ...
    $('.guapi').change(function () {

    console.log($('#da-category').val());

    if($('#da-category').val()=='自定义'){
    ...
    function check_input() {
        ...
        var sUrl = $( '#da-url' ).val();
        var sCategory = $('#da-category').val();
        var sDescribe = $('#da-describe').val();
    ...
    function click_edit_button( curObj ) {
        ...
        var sCategory = obj.find('a.category').text();
        var sDescribe = obj.find('a.describe').text();
        ...
        $( '#da-url' ).val( sUrl );
        $('#da-category').val(sCategory);
        $('#da-describe').val(sDescribe);
