...
class DX_Seo_Do_Anchor_Datas {
    ...
    function __construct() {
        ...
        add_action('wp_ajax_' . DXSEO_PRE .'_anchor_generate',array($this,'ajax_anchor_generate'));
    }
    ...
    function do_search() {
        if( $this->search_word ) {
            foreach( $this->datas as $key => $data ) {
                if( stripos( $key, $this->search_word ) === false )//$data[1]
    ...
    function refresh_table() {
        ...
        foreach( $this->datas as $key => $data ):
            ...
            $describe = isset($data[2]) ? $data[2]:'';
            $nofollow = isset( $data[3] ) ? $data[3] : 0;
            $blank = isset( $data[4] ) ? $data[4] : 1;
            $ignore = isset( $data[5] ) ? $data[5] : 1;
            $strong = isset( $data[6] ) ? $data[6] : 0;
            $num = isset( $data[7] ) ? $data[7] : 1;
            $priority = isset( $data[8] ) ? $data[8] : 10;
            $generate = isset($data[9]) ? $data[9] :'';
            ?>
            <tr id="anchor-<?php echo $serial; ?>" class="item" valign="top">
                ...
                <td class="column-category"><a class="category" href="" ><?php echo $category; ?></a></td>

                <td class="column-describe"><a class="describe" href=""><?php echo $describe==''?'无':$describe; ?></a></td>
                ...
                <td class="column-category"><input <?php if($generate==''){echo 'style="color:#0073aa"';}else{echo 'style="color:#" disabled="true"';}?> type="button" name="da-button-add" id="da-button-add" class="generate_topic_pages" value="生成专题页" /></td>
            </tr>
            <?php
            $serial++;
        endforeach;
    ...
    function sanitize_input( $input, $type ) {
        switch( $type ) {
            ...
            case 'category': $input = (string)($input);break;
            case 'url': $input = (string) esc_url( $input ); break;
            case 'describe':$input=(string)($input);break;
    ...
    function ajax_anchor_update() {
        ...
        $category = isset($_POST['category']) ? $this->sanitize_input( $_POST['category'],'category' ): '';//分类
        $describe = isset($_POST['describe']) ? $this->sanitize_input($_POST['describe'],'describe'):'';//描述
        ...
        if( ! empty( $keyword ) && ! empty( $url ) ) {
            ...
            if($describe=='无'){$describe='';}
            $this->datas[ $keyword ] = array( $url,$category,$describe, $nofollow, $blank, $ignore, $strong, $num, $priority );
    ...
    function ajax_anchor_generate()
    {
        ...
        try{
            ...
            $data = [
                ...
                'post_status'=>'publish',
                'post_content'=>$this->datas[$keyword][2],//json_encode(['keywords'=>$keyword,'category'=>$this->datas[$keyword][1],'content'=>$this->datas[$keyword][2]]),
                'post_title'=>$keyword,
                'post_type'=>'case',//专题
                'post_excerpt'=>$this->datas[$keyword][1]

            ];

            $create_post  = wp_insert_post($data);

            if($create_post==0){

                wp_die(json_encode(['code'=>400,'message'=>'生成失败']));

            }

            $this->datas[ $keyword ][0]='http://wp.yiyefei.me/case/'.$create_post.'.html';

            $this->datas[ $keyword ][9]=1;
...
//            wp_die(['code'=>200,'message'=>'生成成功']);
    ...
    function ajax_anchor_delete() {
        ...
        $url = $this->datas[$keyword][0];

        $post_id = $this->find_um($url);
        ...
        if( $this->datas && $keyword && isset( $this->datas[ $keyword ] ) ) {
            ...
            $wpdb->delete('wp_posts',['ID'=>$post_id]);//删除关键词生成的专题页
